...
// Tip: pass a non-visible label (e.g. "##hello") then you can use the space to active other text or image.
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // We don't have CPU clipping primitives to clip the CloseButton (until it becomes a texture), so need to add an extra active call (temporary in the case of vertical animation)
    ...
    // Restore main window position so user can active there
    if (want_clip_rect)
        PopClipRect();
    window->DC.CursorPos = backup_main_cursor_pos;
